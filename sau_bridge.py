...
ACCOUNT_FILE = Path(__file__).parent / "data" / "biliup-account.json"
...
def upload(video_path: str, title: str, desc: str, tags: list,
    ...
    Pexels 素材按「转载」类型投稿并注明来源（--copyright 2），合规优先。
    """
    args = ["-u", str(ACCOUNT_FILE), "upload", video_path,
            "--title", title, "--desc", desc, "--tid", str(tid),
            "--tag", ",".join(tags),
            "--copyright", "2", "--source", source_url or "https://www.pexels.com"]
    if cover:
        args += ["--cover", cover]
    result = _run(args)
